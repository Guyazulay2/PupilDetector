...
class gaze_inferer(object):
    ...
    def process(self, video_src, mode, output_record_path="", batch_size=32,
                output_video_path="", heatmap=False, print_prefix="", model_name="weights", headers=None):
        """
        ...
        if mode == "Infer":    
            ...
            # Initialize the data according to Triton
            inputs = []
            outputs = []
            inputs.append(httpclient.InferInput('INPUT0', [1, 4], "FP64"))
            ...
            Y_batch = results.as_numpy('OUTPUT0')
            #Y_batch = self.model.predict(X_batch) # one frame --> in one batch --> in one video        
            print("\n************shapes are:************\n", X_batch.shape, ", ", X_batch[0].shape, "\n")
            print("\n************datatype:************\n", type(np.max(X_batch[0])))
            print("\n************before conversion:************\n")
            print("\nmax is: ", np.max(X_batch[0]), ", min is: ", np.min(X_batch[0]))
            print("\n************after conversion:************\n")
            print("\nmax is: ", np.max(X_batch[0] * 255), ", min is: ", np.min(X_batch[0] * 255))            
            
            print("\n************shapes are:************\n", Y_batch.shape, ", ", Y_batch[0].shape, "\n")
            print("\n************datatype:************\n", type(np.max(Y_batch[0])))
            print("\n************before conversion:************\n")
            print("\nmax is: ", np.max(Y_batch[0]), ", min is: ", np.min(Y_batch[0]))
            print("\n************after conversion:************\n")
            print("\nmax is: ", np.max(Y_batch[0] * 255), ", min is: ", np.min(Y_batch[0] * 255))
            image = (Y_batch[0] * 255).astype(np.uint8)
            plt.imsave(output_video_path, image)
            return

            _, _, _ = self.infer_image(X=X_batch, Y=Y_batch, img_id=str(video_src[video_src.rfind('/') + 1 : video_src.rfind('.')]), csv_path=str(output_record_path), out_path=output_video_path)
...
#                     vid_frame = draw_ellipse(output_frame=vid_frame, frame_shape=vid_frame_shape_2d, ellipse_info=ellipse_info, color=[255, 255, 0])
    ...
    def infer_image(self, X, Y, img_id, csv_path, out_path=None):
        csv_dict = dict()
        ...
        if (centre is not None) and self.infer_gaze_flag:
            ...
            if out_path is not "" and out_path is not None:
            ...
            result = { str(img_id): [str(ellipse_centre_np[0]) + "," + str(ellipse_centre_np[1])] }
