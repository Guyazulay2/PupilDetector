...
class gaze_inferer(object):
    def __init__(self, model, flen=6, ori_video_shape=(240,320), sensor_size=(3.6,4.8), infer_gaze_flag=True):
        """
    ...
    def http_image_inference(self, image, result_path="./inference.json"):
            ...
            outfile = open(str(result_path), 'rw')
            json.dump(inference_dict, outfile)
    ...
    def process(self, input_src, mode="Infer", output_record_path="", batch_size=32,
        ...
        if mode == "Infer":
            ...
            _, _, _, _ = self.infer_image(X=X_batch, Y=Y_batch, img_id=str(input_src[input_src.rfind('/') + 1 : input_src.rfind('.')]), csv_path=str(output_record_path), out_path=str(output_path))
